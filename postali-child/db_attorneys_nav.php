...
$word_nav = preg_replace("/[^A-Za-z0-9]/", " ", $_GET['word']);
$search_type_nav = $_GET['search_type'];
...
if (strlen($word_nav) >= 1 && $word_nav !== ' ') {
    ...
    $sql = 'SELECT attorneypost.post_title as attorney_name,lastName.meta_value as attorney_last_name, attorneypost.guid as attorney_guid,  attorneypost.post_name as attorney_info 
            ...
            WHERE attorneypost.post_title like "%' . $word_nav . '%"
    ...
    if (count($result) > 0) {
        foreach ($result as $row) {
            ...
            $display_name = preg_replace("/" . $word_nav . "/i", "<b class='highlight'>" . $word_nav . "</b>", $row->attorney_name);
